...
const API_URL = "http://172.17.33.130:5000/predict-house"; // Update the endpoint for house prediction

const validationSchema = Yup.object().shape({
  location: Yup.string().required("Location is required"),
  size: Yup.number()
    .required("Size (sq ft) is required")
    .min(100, "Size should be at least 100 sq ft"),
  bedrooms: Yup.number()
    ...
    const htmlContent = `
      <html>
        <body style="margin: 40px; font-family: Arial, sans-serif; text-align: center; border: 2px solid #000;">
          ...
          <h1 style="margin-bottom: 20px;">House Price Prediction Report</h1>
          <p><strong>Date:</strong> ${currentDate}</p>
          <hr style="margin-bottom: 20px;"/>
          ${imageBase64 ? `<img src="${imageBase64}" alt="House Image" style="width: 50%; height: auto; margin-bottom: 20px;" />` : ""}
          <p><strong>Location:</strong> ${values.location}</p>
          <p><strong>Size:</strong> ${values.size} sq ft</p>
      ...
      <View style={styles.container}>
        ...
        >
          {({
            ...
            <View>
              <Text style={styles.heading}>House Price Prediction</Text>
              ...
              <TextInput
                label="House Type (e.g., Apartment, Villa)"
                style={styles.input}
