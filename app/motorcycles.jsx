...
const API_URL = "http://172.17.33.130:5000/predict_motorcycle";

const validationSchema = Yup.object().shape({
  name: Yup.string().required("Motorcycle name is required"),
...
const MotorcyclePriceForm = () => {
    ...
    const htmlContent = `
      <html>
        <body style="margin: 40px; font-family: Arial, sans-serif; text-align: center; border: 2px solid #000;">
          ...
          <h1 style="margin-bottom: 20px;">Motorcycle Price Prediction Report</h1>
          <p><strong>Date:</strong> ${currentDate}</p>
          <hr style="margin-bottom: 20px;"/>
          ${imageBase64 ? `<img src="${imageBase64}" alt="Motorcycle Image" style="width: 50%; height: auto; margin-bottom: 20px;" />` : ""}

          <p><strong>Motorcycle Name:</strong> ${values.name}</p>
  ...
  const handleSubmit = (values, { setSubmitting }) => {
    ...
    if (!values.name.trim()) {
      setErrorMessage("Invalid motorcycle name. Please enter a valid brand.");
      setSubmitting(false);
      return;
    }
      ...
      <View style={styles.container}>
        ...
        >
          {({
            ...
            <View>
              <Text style={styles.heading}>Motorcycle Price Prediction</Text>
...
export default MotorcyclePriceForm;
